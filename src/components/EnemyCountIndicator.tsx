...
import { BLOCK_SIZE, FIELD_SIZE } from 'utils/consts';
...
export default class EnemyCountIndicator extends React.PureComponent<P, {}> {
  render() {
    ...
    return (
      <g data-role="remaining-enemy-count-indicator" transform={transform}>
        {_.range(count).map(t => <EnemyTankThumbnail key={t} x={8 * (t % 2)} y={8 * Math.floor(t / 2)} />)}
      </g>
    );
  }
}
